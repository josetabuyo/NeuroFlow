...
const CONFIG_ROWS: ConfigRow[] = [
  ...
  { section: "Connection › on (intra-region wiring)", key: "shape", type: "hard", desc: "deamon.shape — daemon footprint shape (square, crown). Default crown" },
  { section: "Connection › on (intra-region wiring)", key: "centroid", type: "hard", desc: "deamon.centroid — daemon center placement" },
  { section: "Connection › on (intra-region wiring)", key: "groups", type: "hard", desc: "deamon.groups — array of dendrite groups; each is {id, weight?, offset?, weights, density?, noise?, sectors?, learning?}" },
  ...
  { section: "Connection › on (intra-region wiring)", key: "weight", type: "hard", desc: "groups[].weight — this group's dendrite weight; its sign is the polarity. Defaults to +1 for a single dendrite, -1 once sectors make it partitioned" },
  ...
  { section: "Connection › on (intra-region wiring)", key: "gap", type: "hard", desc: "deamon.gap — default empty-ring gap (int) used to resolve groups[].offset when a group omits it. Default 0 (adjacent)" },
  { section: "Connection › on (intra-region wiring)", key: "offset", type: "hard", desc: "groups[].offset — ring this group starts at. Optional: omitted → previous group's outer ring + gap + 1 (first group with no offset → gap + 1, i.e. 1 by default)" },
  { section: "Connection › on (intra-region wiring)", key: "sectors", type: "hard", desc: "groups[].sectors — number of angular sectors; presence marks this group as partitioned" },
