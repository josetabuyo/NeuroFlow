...
const CONFIG_ROWS: ConfigRow[] = [
  ...
  { section: "Connection › on (intra-region wiring)", key: "groups", type: "hard", desc: "deamon.groups — array of dendrite groups; each is {id, weight?, gap?, weights, density?, noise?, sectors?, learning?}" },
  ...
  { section: "Connection › on (intra-region wiring)", key: "gap", type: "hard", desc: "deamon.gap (default) or groups[].gap (per-group override) — empty-ring gap (int) between this group and the previous one. Default 0 (adjacent). A group's own gap wins over deamon.gap for its own transition. Never applies before the first group, which always starts at ring 1 regardless of gap. offset is no longer settable directly — every group's start is computed from gap" },
